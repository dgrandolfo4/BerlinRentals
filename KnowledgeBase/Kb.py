            ...
            try:
                for row in self.dataframe.iterrows():
                    ...
                    # List
                        ...
                        cleaned_list = str(value[features[i]]).replace('[', '').replace(']', "").replace('"','').replace("'",'').replace("\\", '').split(",")
...
def main():
    try:
        dataframe = pd.read_csv('./datasets/prolog_dataframe.csv')
        kb = KnowledgeBase(dataframe)
        kb.KbCreation()
        kb.RulesCreation()
        print("Knowledge Base eseguito.")
